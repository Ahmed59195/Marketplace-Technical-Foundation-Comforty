...
const SearchPage = () => {
  const [, setProducts] = useState<Product[]>([]); // Set products state correctly
  ...
  useEffect(() => {
    // Fetch product data (mock data in this case)
    const fetchProducts = async () => {
      ...
      if (query) {
        ...
        setFilteredProducts(data); // If no query, show all products
      }
    };
            ...
            <div key={product._id} className="bg-white p-4 rounded shadow-md">
              <div className="h-64 overflow-hidden mb-4">
                <Image
                  ...
                  width={500} // Added width and height for the image component
                  height={500}
                />
              </div>
